...
export class TicTacToeGame {
    ...
    constructor(players: string[]) {
        ...
        if (this.players.length != 2)
            throw 'Not exactly 2 players: ' + players;
        if (this.players[0] == this.players[1])
            throw 'Both players are the same: ' + players;
    ...
    public act(player: string, action: {x: number, y: number}) {
        ...
        if (player != this.players[this.whoseTurn])
            throw 'Invalid player for move: ' + player;

        if (this.boardState[x][y] != TileState.Blank)
            throw 'Cannot play on a non-blank space';
    ...
    private updateStatus() {
        ...
        for (let coords of winners) {
            let tiles = coords.map(pair => this.boardState[pair[0]][pair[1]]);
